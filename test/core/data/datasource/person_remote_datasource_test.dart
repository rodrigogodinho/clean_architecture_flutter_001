import 'package:clean_architecture_flutter_001/core/person/data/datasource/person_remote_datasource.dart';
...
void main() {
  ...
  setUpAll(() {
    client = ClientMock();
    dataSource = PersonRemoteDataSource(client);
  });

  test("Testando o remote datasource", () async {
    when(() => client.get(Uri.parse(PersonRemoteDataSource.URL_GET)))
        .thenAnswer(
            (invocation) async => http.Response(jsonReturn, io.HttpStatus.ok));

    final list = await dataSource.getPersonList();
    expect(list, allOf([isNotEmpty, isA<List<PersonDTO>>()]));
